...
class CartNotifier extends StateNotifier<List<CartItem>> {
  ...
  void addToCart(FoodItem item) {
    ...
    if (index == -1) {
      ...
      final updated = [...state];
      updated[index] = CartItem(item: item, quantity: updated[index].quantity + 1);
      state = updated;
    }
    _saveCartToPrefs();
  }
  ...
  void increaseQuantity(String itemId) {
    final updated = state.map((cartItem) {
      if (cartItem.item.id == itemId) {
        return CartItem(item: cartItem.item, quantity: cartItem.quantity + 1);
      }
      return cartItem;
    }).toList();
    state = updated;
  ...
  void decreaseQuantity(String itemId) {
    final updated = state.where((cartItem) {
